...
class RedisServiceBehavior extends Behavior
{
    public $redis;
     ...
     * @return Boolean
     */
    public function initHSet($values, $keyInfo)
    {
        $result = false;
        $key = $keyInfo['key'];
        if (!$this->keyExists($key) && !empty($values)) {
     ...
     * @return Boolean
     */
    public function getHSetAll($keyInfo)
    {
        $result = [];
        $key = $keyInfo['key'];
        if ($this->keyExists($key)) {
     ...
     * @return Boolean
     */
    public function getHSet($field, $keyInfo)
    {
        $result = '';
        $key = $keyInfo['key'];
        if ($this->keyExists($key)) {
     ...
     * @return Boolean
     */
    public function updateValueToHSet($field, $value, $keyInfo)
    {
        $result = false;
        $key = $keyInfo['key'];
        if ($this->keyExists($key)) {
